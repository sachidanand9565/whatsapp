...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  await query(
    ...
    [name, price_monthly, price_yearly, max_contacts, max_messages_per_month, max_agents, JSON.stringify(features || []), is_active ? 1 : 0, params.id]
  )
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  if (!getAdminFromCookies()) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  await query('DELETE FROM admin_plans WHERE id = ?', [params.id])
  return NextResponse.json({ ok: true })
}
